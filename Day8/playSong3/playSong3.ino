...
int wholeNote = 1000; // change pace of music("speedy")
int song = 1;    //select a song
int buzzPin = 8;
int switchPin = 12;
int potPin = A0;
int nsong = 3;
...
void setup() {pinMode(switchPin,INPUT_PULLUP);}
...
void singsong(int dur[], int mel[]) {
  ...
  for (int thisNote = 0; dur[thisNote] != 0; thisNote++) {
    ...
    int curSwitch = digitalRead(switchPin);
    if(curSwitch != prevSwitch) delay(5);
  ...
  prevSwitch = curSwitch;
    wholeNote = analogRead(potPin) * 3;
    int noteDuration = wholeNote / dur[thisNote];
